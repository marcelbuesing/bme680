...
impl I2CUtil {
    pub fn read_byte<I2C>(i2c: &mut I2C, dev_id: u8, reg_addr: u8) -> Result<u8, Error<I2C::Error>>
    where
        I2C: I2c,
    {
        let mut buf = [0; 1];
    ...
    I2C: I2c,
{
    pub fn soft_reset(
        ...
        i2c.write(dev_id.addr(), &tmp_buff).map_err(Error::I2C)?;
    ...
    pub fn init(
        ...
        delay: &mut D,
        dev_id: I2CAddress,
    ) -> Result<Bme680<I2C, D>, Error<I2C::Error>> {
    ...
    fn bme680_set_regs(&mut self, reg: &[(u8, u8)]) -> Result<(), Error<I2C::Error>> {
        if reg.is_empty() || reg.len() > (BME680_TMP_BUFFER_LENGTH / 2) {
    ...
    ) -> Result<SensorSettings, Error<I2C::Error>> {
    ...
    pub fn get_sensor_mode(&mut self) -> Result<PowerMode, Error<I2C::Error>> {
        let regs =
            I2CUtil::read_byte(&mut self.i2c, self.dev_id.addr(), BME680_CONF_T_P_MODE_ADDR)?;
    ...
    pub fn bme680_set_profile_dur(&mut self, tph_sett: TphSett, duration: Duration) {
        ...
        let millis = (duration.as_secs() * MILLIS_PER_SEC)
    ...
    ) -> Result<Duration, Error<I2C::Error>> {
        let os_to_meas_cycles: [u8; 6] = [0u8, 1u8, 2u8, 4u8, 8u8, 16u8];
        // TODO check if the following unwrap_ors do not change behaviour
    ...
    fn get_calib_data<I2CX>(
        i2c: &mut I2CX,
        dev_id: I2CAddress,
    ) -> Result<CalibData, Error<I2C::Error>>
    where
        I2CX: I2c,
        Error<<I2C as I2cErrorType>::Error>: From<Error<<I2CX as I2cErrorType>::Error>>,
    {
    ...
    fn set_gas_config(&mut self, gas_sett: GasSett) -> Result<(), Error<I2C::Error>> {
    ...
    fn get_gas_config(&mut self) -> Result<GasSett, Error<I2C::Error>> {
    ...
    pub fn get_sensor_data(
        &mut self,
        delay: &mut D,
    ) -> Result<(FieldData, FieldDataCondition), Error<I2C::Error>> {
